...
class LocalProxy:
    ...
    async def handle_client(
        ...
        try:
            request = await self._read_http_headers(
                local_reader,
            )

            hostname, port = self._parse_connect(
                request,
            )
    ...
    def _parse_connect(
        ...
        hostname, port = target.decode().rsplit(
            ":",
            1,
        )
